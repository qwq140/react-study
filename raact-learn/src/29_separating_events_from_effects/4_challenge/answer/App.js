...
function ChatRoom({ roomId, theme }) {
    ...
    // Effect Event의 roomId는 Effect Event가 호출되는 시점의 값이다.
    // "general" -> "travel" -> "music" 으로 빠르게 전환하면 "travel"방의 알림을 보여줄 때는 이미 roomId가 "music"이다.
    const onConnected = useEffectEvent((visitedRoomId) => {
        showNotification(visitedRoomId + '에 오신 것을 환영합니다', theme);
    });

    // 나중의 알림만 보여주는 방법
    // useEffect 내부에서 timeout ID를 저장하고 클린업 함수에서 clear 하면 된다.
    useEffect(() => {
